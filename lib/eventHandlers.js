const { go, each, map, every, some, filter, head } = _;
...
export function eventHandlers() {
  ...
  $account_window
    ...
    .on("click", "#login_btn", async (e) => {
      ...
      if (input_is_null) {
        ...
        //input null이 없는 경우
        //유저 계정 존재 여부 체크
        if (!isUserExist(input_user_name)) {
          //유저 계정 없는 경우
          ...
          if (
            ...
            //패스워드 일치하는 경우
            const user_id = head(
              readData(DB_accounts, { user_name: input_user_name }, "id")
            );
            //세션에 유저 id 등록
            createSessionData(DB_session_user, { user_id: user_id });
            //_todo 아이템 불러오기
            readTodoItems(user_id);
            showWelcomeMessage(
              $account_control.find("#main_welcome"),
              input_user_name
            );
    ...
    .on("click", "#signup_btn", async (e) => {
      ...
      if (input_is_null) {
        ...
        if (input_pw !== input_pw_check) {
          ...
          //패스워드 입력 일치하는 경우
          if (isUserExist(signup_user_name)) {
            //아이디 기존 등록 여부 확인
  ...
  $account_control.on("click", "#main_logout", () => {
    ...
    //세션 로그인 정보 제거
    deleteSessionData(DB_session_user);
    statusUpdate($todo_window);
  });
}
export function readTodoItems(user_id) {
  ...
  const $todo_window = $("#todo_window");
  const $content = $todo_window.find("section#content");
  const user_name = head(
    readData(DB_accounts, { id: user_id }, "user_name")
  );
  ...
  statusUpdate($todo_window);
  showWelcomeMessage($account_control.find("#main_welcome"), user_name);
}
...
function isUserExist(username) {
  return go(
    readData("todo_accounts", null, "user_name"),
    some((DB_user) => DB_user === username)
  );
}

function showWelcomeMessage($el, message) {
  $el.text(`Welcome, ${message}!`);
}
