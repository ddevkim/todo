const { go, each, map, every, some } = _;
  ...
  $register_item
    ...
    .on("keydown", "#item_input", (e) => {
      ...
      if (input_text && e.keyCode === 13) {
        ...
        //DB에 추가 아이템 쓰기
        const appended_data = appendData(DB_items, item_content);
        //화면에 추가 아이템 보여주기
        append($content)(todoTmpl(appended_data));
...
export function readTodoItems(user_id) {
  ...
  const todo_items = readData(DB_items, { user_id: user_id });
  const $todo_window = $("#todo_window");
  const $account_control = $("#account_control");
  const $content = $todo_window.find("section#content");

  go(todo_items, map(todoTmpl), each(append($content)));
