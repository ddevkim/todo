...
export function eventHandler(item_list_data) {
  ...
  $account
    .on("click", "#login_btn", async (e) => {
      ...
      if (input_null_check) {
        ...
        if (!is_id_exist) {
          ...
          if (
            user_account_data.user_pw !== (await encryptPassword($login_user_pw.val().toString()))
          ) {
    ...
    .on("keydown", "#signup_user_pw, #signup_user_pw_check", (e) => {
      $pw_check_label
        .css("color", "#6a6969")
    ...
    .on("click", ".delete_btn", (e) => {
      ...
      if ($delete_item.hasClass("completed") || confirm("Not over yet. Wanna remove?")) {
        animateHide($delete_item, 500, () => {
          $delete_item.remove();
          todo_items_data["item_list"] = removeData(todo_items_data["item_list"], {
            id: Number($delete_item.attr("data-id")),
          });
          statusUpdate($todo_window);
        });
      }
  ...
  $account_control.on("click", "#main_logout", (e) => {
...
export function statusUpdate($main_el, doFilter = true) {
  ...
  switch (filter_category) {
    ...
    case "Completed": {
      numberIndicator($left_items_counter, complete_num, "Complete", false);
      break;
    }
  }
  left_num < 1
...
function numberIndicator($el_indicator, num, text, is_done_show) {
  if (num < 1) {
    is_done_show ? $el_indicator.text("All Done!").css("color", "#248d4c") : $el_indicator.text("");
  } else {
